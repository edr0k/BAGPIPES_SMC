...
#fit_instructions["veldisp"] = (0.,500.)
...
df = pd.read_csv('data/phot_100_all.csv')
...
fit_cat = pipes.fit_catalogue(IDs, fit_instructions, load_data_splus, spectrum_exists=False,
                              cat_filt_list=filter_list_splus, run="phot_100_all_ebvfit_burst", make_plots=True,
                              full_catalogue=True)

fit_cat.fit(verbose=False)

results_cat = Table.read('pipes/cats/phot_100_all_ebvfit_burst.fits', format='fits')
...
results_cat.write('pipes/cats/phot_100_all_ebvfit_burst.csv', format='csv', overwrite=True)
